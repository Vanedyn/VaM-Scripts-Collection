...
namespace MorphMerger
{
    public class MorphMerger : MVRScript
    {
        ...
        private UIDynamicButton _setToDefaultButton;
        ...
        public override void Init()
        {
            try
            {
                pluginLabelJSON.val = "MorphMerger v1.0.4 (by ProjectCanyon)";
                ...
                _setToDefaultButton = CreateButton("Reset Morph(s)", rightSide: true);
                _setToDefaultButton.buttonColor = Color.yellow;
                if (_setToDefaultButton != null)
                    _setToDefaultButton.button.onClick.AddListener(() =>
                    {
                        SetBankToDefault(_mainBank);
                        SetBankToDefault(_genitalBank);
                    });
        ...
        private void SetBankToDefault(IEnumerable<SelectableMorph> bank)
        {
            try
            {
                if (bank == null)
                ...
                foreach (SelectableMorph selectableMorph in bank)
                {
                    ...
                    {
                        selectableMorph.Morph.animatable = false;
                        selectableMorph.Morph.ResetRange();
                        selectableMorph.Morph.SetValue(selectableMorph.Morph.jsonFloat.defaultVal);

                        for (int index = 0; index < _characterSelector.morphsControlUI.morphNames.Length; ++index)
                        {
                            string[] strArray = _characterSelector.morphsControlUI.morphNames[index].Split(':');
                            if (strArray.Length == 3 && string.Equals(strArray[1], selectableMorph.Morph.morphName, StringComparison.InvariantCultureIgnoreCase))
                            {
                                _characterSelector.morphsControlUI.morphEnabled[index] = false;
                                break;
                            }
                        }
                    }
                ...
                _characterSelector.morphsControlUI.ResyncMorphs();
            }
            catch (Exception e)
            {
                SuperController.LogError("Exception caught: " + e);
        ...
        private void Merge()
        {
            try
            {
                ...
                if (mainMorph != null)
                {
                    string mainMetaPath = characterSelector.morphBank1.autoImportFolder + $"/{morphName}.vmi";
                    string mainDeltasPath = characterSelector.morphBank1.autoImportFolder + $"/{morphName}.vmb";
                ...
                if (genMorph != null)
                {
                    string genMetaPath = characterSelector.morphBank2.autoImportFolder + $"/{morphName}-Genital.vmi";
                    string genDeltasPath = characterSelector.morphBank2.autoImportFolder + $"/{morphName}-Genital.vmb";
        ...
        public class SelectableMorph
        {
            ...
            public float NormalisedMagnitude =>
                Morph.morphValue > Morph.jsonFloat.defaultVal ?
